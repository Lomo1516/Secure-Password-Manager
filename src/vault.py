...
# ── Hide the terminal window when launched directly ──────────────────────────
...
class EntryDetailScreen(ctk.CTkToplevel):
    ...
    def _build(self):
        ...
        if tags:
            make_section_label(card, "TAGS", pady=(10, 0))
            tag_wrap = ctk.CTkFrame(card, fg_color="transparent")
            tag_wrap.pack(fill="x", padx=20, pady=(4, 0))
            for tag in tags:
                ctk.CTkLabel(
                    tag_wrap, text=f"  {tag}  ",
                    font=FONT_BODY_SM, text_color=BLUE,
                    fg_color=BLUE_LIGHT, corner_radius=8,
...
class AddEntryDialog(ctk.CTkToplevel):
    ...
    def _build(self, name: str, data: dict):
        ...
        make_section_label(card, "TAGS  (optional — comma separated, e.g. work, social)", pady=(14, 0), padx=22)
        self.var_tags = ctk.StringVar(value=", ".join(data.get("tags", [])))
        self.field_tags = make_modern_entry(card, self.var_tags, height=48, placeholder="e.g. work, finance, social")
        self.field_tags.pack(fill="x", padx=22, pady=(6, 0))
    ...
    def _save(self):
        ...
        if not pw:
            ...
            return
        try:
            raw_tags = [t.strip() for t in self.var_tags.get().split(",") if t.strip()]
            self.on_save(name, {
...
class VaultHomeScreen(ctk.CTkToplevel):
    ...
    def _build(self):
        ...
        self.search_entry = make_modern_entry(
            search_inner,
            self.search_var,
            placeholder="Search by account, username or tag…",
            height=44,
        )
        self.search_entry.pack(fill="x", pady=(6, 0))

        # Tag filter
        tag_filter_wrap = ctk.CTkFrame(search_inner, fg_color="transparent")
        tag_filter_wrap.pack(fill="x", pady=(8, 0))
        ctk.CTkLabel(
            tag_filter_wrap, text="Filter by tag:",
            font=FONT_SECTION, text_color=MID_GREY, fg_color="transparent",
        ...
        self.tag_filter_menu = ctk.CTkOptionMenu(
            ...
            values=["All"],
            fg_color=LIGHT_GREY, button_color=BLUE, button_hover_color=BLUE_DARK,
        ...
        ctk.CTkButton(
            toolbar, text="⚙ Generator",
            width=130, height=76, corner_radius=20,
            fg_color=WHITE, hover_color=BLUE_LIGHT,
            text_color=BLUE, font=FONT_BTN_LG,
            border_width=2, border_color=DIVIDER,
    ...
    def _render_rows(self):
        ...
        # Refresh tag options
        all_tags = sorted({t for d in self.vault.values() for t in d.get("tags", [])})
        tag_options = ["All"] + all_tags
        self.tag_filter_menu.configure(values=tag_options)
        ...
        entries = sorted(
            (
                ...
                if (query in n.lower() or query in d.get("username", "").lower()
                    or query in d.get("website", "").lower()
                    or any(query in t.lower() for t in d.get("tags", [])))
                and (active_tag == "All" or active_tag in d.get("tags", []))
            ),
    ...
    def _make_row(self, name: str, data: dict, index: int):
        row = make_soft_card(self.list_frame, fg_color=OFF_WHITE, corner_radius=16, height=82)
        ...
        tags = data.get("tags", [])
        if tags:
            tag_row = ctk.CTkFrame(text_wrap, fg_color="transparent")
            tag_row.pack(anchor="w", pady=(2, 0))
            for tag in tags[:4]:  # show up to 4 tags inline
                ctk.CTkLabel(
                    tag_row, text=f" {tag} ",
                    font=(FONT_FAMILY, 10, "bold"), text_color=BLUE,
                    fg_color=BLUE_LIGHT, corner_radius=6,
    ...
    def _open_generator_dialog(self):
        ...
        dlg.configure(fg_color=OFF_WHITE)
        dlg.transient(self)
        dlg.grab_set()
        center_to_parent(dlg, self, 420, 360)
        dlg.after(20, lambda: bring_to_front(dlg))

        hdr = make_soft_card(dlg, fg_color=BLUE, corner_radius=0)
        hdr.pack(fill="x")
        ctk.CTkLabel(hdr, text="Password Generator",
                     font=FONT_TITLE, text_color=WHITE,
                     fg_color="transparent").pack(anchor="w", padx=22, pady=(18, 4))
        ctk.CTkLabel(hdr, text="Generate a strong random password.",
                     font=FONT_BODY, text_color="#D9E8F7",
                     fg_color="transparent").pack(anchor="w", padx=22, pady=(0, 14))

        panel = PasswordGeneratorPanel(dlg)
        panel.pack(fill="x", padx=20, pady=18)
        panel._toggle()  # expand by default in standalone dialog
...
class MasterPasswordScreen(ctk.CTk):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(hero, text="🔐", font=FONT_ICON_LG, fg_color="transparent").pack(pady=(30, 6))
        ctk.CTkLabel(
            hero,
            text="The Vault",
            font=FONT_HERO, text_color=WHITE, fg_color="transparent",
        ...
        ctk.CTkLabel(
            hero,
            text=("Create your secure vault with a strong master password."
                  if self.is_new_vault
                  else "Welcome back. Enter your master password to unlock."),
            font=FONT_BODY, text_color="#D9E8F7", fg_color="transparent",
